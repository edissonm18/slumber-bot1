...
↩ *Escribe "inicio"* para regresar al menú principal.`);
  ...
  if (state?.flujo === 'pago') {  const choice = (text || '').trim();
    ...
    if (state.pagoDetalle) {
      ...
      if (state.pagoDetalle === 'contraentrega' && state.pagoStep === 'confirmar') {
        if (text === '1') {
...
// Permitir volver al menú principal
      ...
      return sendMessage(from, '⚠️ Por favor, elige *1*, *2*, *3* o *4*, o escribe *"inicio"* para regresar al menú principal.');
    }
  }
